...
export default function EstadisticasPage() {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center max-w-sm px-6">
          ...
          <h2 className="text-lg font-semibold mb-2" style={{ color: '#1a1814' }}>Plan Agencia requerido</h2>
          <p className="text-sm mb-6" style={{ color: '#7a7468' }}>
            Las estadísticas avanzadas están disponibles en el plan Agencia.
          </p>
          <Link href="/pricing"
            className="inline-flex px-5 py-2.5 rounded-xl text-sm font-semibold transition-all"
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto space-y-6">
      <div>
        <h1 style={{ color: '#1a1814' }}>Estadísticas</h1>
        <p className="text-sm mt-1" style={{ color: '#7a7468' }}>Resumen de la actividad de tu dashboard</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={kpi.label} className="rounded-xl p-5"
            ...
            <p className="text-xs font-medium mb-1" style={{ color: '#7a7468' }}>{kpi.label}</p>
            <p className="text-3xl font-bold" style={{ color: '#1a1814' }}>{kpi.value}</p>
            <p className="text-xs mt-1" style={{ color: kpi.subColor ?? '#7a7468' }}>{kpi.sub}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="rounded-xl p-5 space-y-4"
          style={{ background: '#fff', border: '1.5px solid rgba(200,169,110,0.18)' }}>
          <h2 className="text-sm font-semibold" style={{ color: '#1a1814' }}>Por estado</h2>
          <div className="space-y-3">
            {Object.entries(ESTADO_LABEL).map(([key, label]) => (
              <div key={key}>
                <div className="flex justify-between mb-1">
                  <span className="text-xs" style={{ color: '#7a7468' }}>{label}</span>
                </div>
        ...
        <div className="rounded-xl p-5 space-y-4"
          style={{ background: '#fff', border: '1.5px solid rgba(200,169,110,0.18)' }}>
          <h2 className="text-sm font-semibold" style={{ color: '#1a1814' }}>Temperatura (score)</h2>
          <div className="space-y-4">
            {[
              ...
              <div key={item.label}>
                <div className="flex justify-between mb-1">
                  <span className="text-xs font-medium" style={{ color: '#1a1814' }}>{item.label}</span>
                  <span className="text-xs" style={{ color: '#7a7468' }}>{item.sub}</span>
                </div>
      ...
      {stats.por_mes.length > 0 && (
        <div className="rounded-xl p-5"
          style={{ background: '#fff', border: '1.5px solid rgba(200,169,110,0.18)' }}>
          <h2 className="text-sm font-semibold mb-4" style={{ color: '#1a1814' }}>Tendencia mensual</h2>
            ...
            {[...stats.por_mes].reverse().map(m => {
              ...
              return (
                <div key={m.mes} className="flex-1 flex flex-col items-center gap-1">
                  <span className="text-xs" style={{ color: '#7a7468' }}>{m.total}</span>
