...
// Cambia el plan de un tenant a mano (red de seguridad si el webhook de
// Stripe falla o no llega) — no toca la suscripción real en Stripe.
...
export async function PATCH(req: NextRequest) {
  ...
  if (!tenantId || !['free', 'pro', 'agencia'].includes(plan)) {
    return NextResponse.json({ error: 'Parámetros inválidos' }, { status: 400 });
  }

  const apiUrl = process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:8000';

  const res = await fetch(`${apiUrl}/admin/tenants/${tenantId}/plan`, {
    method: 'PATCH',
    headers: {
