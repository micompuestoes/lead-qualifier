    ...
    <div className="p-8">
      ...
      {leadsNuevos > 0 && (
        <button onClick={aplicarLeadsNuevos}
          className="w-full mb-6 flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-semibold transition-all animate-fade-up"
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 style={{ color: '#1a1814' }}>Leads</h1>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        <MetricaCard label="Calientes" valor={calientes} total={total} nivel={3}
          color="#b45309" bgColor="rgba(180,83,9,0.08)" barColor="rgba(180,83,9,0.5)" />
        <MetricaCard label="Tibios"    valor={tibios}    total={total} nivel={2}
          color="#9a7a3a" bgColor="rgba(200,169,110,0.1)" barColor="#c8a96e" />
        <MetricaCard label="Fríos"     valor={frios}     total={total} nivel={1}
          color="#3a7a9a" bgColor="rgba(110,168,200,0.08)" barColor="rgba(110,168,200,0.6)" />
      </div>
        ...
        {[
          ...
          <div key={title}>
            ...
            <div className="flex gap-1.5">
              {opciones.map(o => (
                <button key={o} onClick={() => (setter as (v: typeof o) => void)(o)}
                  className="px-3 py-1.5 rounded-lg text-xs font-medium transition-all"
                  style={{
                    background: valor === o ? '#1a1814' : 'rgba(249,245,238,0.8)',
                    color:      valor === o ? '#f5f0e8' : '#7a7468',
                    border:     valor === o ? 'none' : '1px solid rgba(200,169,110,0.25)',
                  }}>
                  {o}
                </button>
              ))}
      ...
      {cargando ? (
        <div className="flex items-center justify-center py-24 flex-col gap-3">
          ...
          <p className="text-sm" style={{ color: '#7a7468' }}>Cargando leads…</p>
        </div>
      ) : error ? (
        <div className="rounded-xl p-6 text-center" style={{ background: 'rgba(180,83,9,0.05)', border: '1px solid rgba(180,83,9,0.15)' }}>
...
function EmptyState({ sinLeads, onLimpiarFiltros }: { sinLeads: boolean; onLimpiarFiltros?: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-24 px-8 text-center animate-fade-up">
      ...
      <h2 className="text-lg font-semibold mb-2" style={{ color: '#1a1814' }}>
        {sinLeads ? 'Aún no hay leads aquí' : 'Ningún lead coincide con los filtros'}
      </h2>
      <p className="text-sm mb-8 max-w-sm leading-relaxed" style={{ color: '#7a7468' }}>
        {sinLeads
...
function MetricaCard({ label, valor, total, nivel, color, bgColor, barColor }: {
  ...
  return (
    <div className="rounded-xl px-5 py-4" style={{ background: '#fff', border: '1.5px solid rgba(200,169,110,0.18)' }}>
      <div className="flex items-center gap-4 mb-3">
        <div className="w-10 h-10 rounded-lg flex items-center justify-center shrink-0" style={{ background: bgColor }}>
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm" style={{ color: '#7a7468' }}>{label}</p>
          <div className="flex items-baseline gap-2">
            <p className="text-2xl font-bold" style={{ color }}>
              {total === 0 ? '—' : valor}
            </p>
            {total > 0 && <span className="text-xs" style={{ color: '#7a7468' }}>{porcentaje}%</span>}
          </div>
        </div>
      </div>
