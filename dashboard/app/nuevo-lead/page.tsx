...
const inputClass = "w-full px-4 py-2.5 rounded-xl text-sm transition-all outline-none disabled:cursor-not-allowed";
const inputStyle = {
  border: '1.5px solid rgba(200,169,110,0.25)',
  background: 'rgba(255,255,255,0.8)',
  color: '#1a1814',
};
const inputFocusStyle = { border: '1.5px solid #c8a96e', boxShadow: '0 0 0 3px rgba(200,169,110,0.1)' };

function Input({ id, name, type = 'text', value, onChange, placeholder, disabled, required }: {
  ...
  return (
    <input id={id} name={name} type={type} value={value} onChange={onChange}
      ...
      style={{ ...inputStyle, ...(focused ? inputFocusStyle : {}), opacity: disabled ? 0.6 : 1 }}
...
function Textarea({ id, name, value, onChange, placeholder, disabled }: {
...
export default function NuevoLeadPage() {
    ...
    return (
      <div className="p-8 max-w-2xl">
        ...
        <div className="animate-reveal-in mb-8">
          <h1 style={{ color: '#1a1814' }}>Lead cualificado</h1>
          <p className="text-sm mt-1" style={{ color: '#7a7468' }}>El agente ha procesado el lead correctamente.</p>
        </div>

        {/* Score + clasificación */}
        <div className="rounded-2xl p-6 mb-6 animate-reveal-in"
          style={{ background: '#fff', border: '1.5px solid rgba(200,169,110,0.2)', animationDelay: '80ms' }}>
          <div className="flex items-center justify-between mb-4">
            <LeadBadge clasificacion={resultado.classification} />
            <span className="animate-score-pop text-4xl font-bold tabular-nums" style={{ color: '#1a1814' }}>
              {scoreVisible}
              <span className="text-lg font-normal" style={{ color: '#7a7468' }}>/10</span>
            </span>
          </div>
          <ScoreBar score={resultado.score} />
          <p className="text-sm mt-4 leading-relaxed" style={{ color: '#1a1814' }}>
        ...
        <div className="mb-6 animate-reveal-in" style={{ animationDelay: '160ms' }}>
          <div className="rounded-xl overflow-hidden" style={{ border: '1.5px solid rgba(200,169,110,0.2)' }}>
            <div className="px-4 py-3 flex items-center gap-2"
              style={{ background: 'rgba(249,245,238,0.7)', borderBottom: '1px solid rgba(200,169,110,0.12)' }}>
              ...
              <span className="text-sm font-semibold" style={{ color: '#1a1814' }}>Email generado</span>
            </div>
        ...
        {resultado.recommended_actions.length > 0 && (
          <div className="rounded-2xl p-6 mb-8 animate-reveal-in"
            style={{ background: '#fff', border: '1.5px solid rgba(200,169,110,0.2)', animationDelay: '240ms' }}>
            ...
            <ul className="space-y-2">
              {resultado.recommended_actions.map((a, i) => (
                <li key={i} className="flex items-start gap-3 text-sm" style={{ color: '#1a1814' }}>
        ...
        <div className="flex gap-3 animate-reveal-in" style={{ animationDelay: '320ms' }}>
          ...
          <Link href={`/leads/${resultado.lead_id}`}
            className="flex-1 text-center px-4 py-2.5 rounded-xl text-sm font-semibold transition-all"
            style={{ border: '1.5px solid rgba(200,169,110,0.3)', color: '#1a1814', background: 'transparent' }}>
            Ver detalle
          </Link>
          <button onClick={resetear}
            className="px-4 py-2.5 rounded-xl text-sm transition-all"
            style={{ border: '1.5px solid rgba(200,169,110,0.25)', color: '#7a7468', background: 'transparent' }}>
            Nuevo
          </button>
        </div>
      </div>
  ...
  return (
    <div className="p-8 max-w-2xl">
      <div className="mb-8">
        <h1 style={{ color: '#1a1814' }}>Cualificar nuevo lead</h1>
              ...
              return (
                <div key={i} className="flex items-center gap-3 transition-opacity duration-500"
                  ...
                  <span className="text-sm font-medium" style={{
                    color: hecho ? '#2d7a3a' : activo ? '#1a1814' : '#7a7468',
                  }}>
                    {paso.label}
                  </span>
                  {activo && (
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div>
          <label htmlFor="name" className="block text-sm font-semibold mb-1.5" style={{ color: '#1a1814' }}>
        ...
        <div>
          <label htmlFor="email" className="block text-sm font-semibold mb-1.5" style={{ color: '#1a1814' }}>
        ...
        <div>
          <label htmlFor="phone" className="block text-sm font-semibold mb-1.5" style={{ color: '#1a1814' }}>
            Teléfono <span className="font-normal" style={{ color: '#7a7468' }}>(opcional)</span>
          </label>
        ...
        <div>
          <label htmlFor="message" className="block text-sm font-semibold mb-1.5" style={{ color: '#1a1814' }}>
